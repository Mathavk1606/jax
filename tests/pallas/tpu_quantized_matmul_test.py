...
from jax.experimental.pallas.ops.tpu.quantized_matmul import quantized_matmul_kernel
from jax.experimental.pallas.ops.tpu.quantized_matmul import tuned_block_sizes
import jax.numpy as jnp

quantized_matmul_int8 = quantized_matmul_kernel.quantized_matmul_kernel
...
def quantize_array(x, n_bits: int = 8, dim: int = -1):
  ...
  x_scale = max_val / int_max
  x_q = jnp.clip(jnp.round(x / x_scale), int_min, int_max).astype(jnp.int8)
  return x_q, x_scale.astype(jnp.float32)


def reference_quantized_matmul(x, w_q, w_scale, quantize_activation=True):
  if quantize_activation:
    x_q, x_scale = quantize_array(x)
...
class QuantizedMatmulKernelTest(jtu.JaxTestCase):
  ...
  def _test_quantized_matmul(
    ...
    x = jax.random.uniform(
        k0, (bs, n_input_features), dtype=dtype, minval=-1, maxval=1
    )
