...
Value _canonicalize_store(const CanonicalizeContext &ctx, Operation &raw_op) {
  ...
  // Upstream cases may have changed the shape of the memref!
  ...
  *(mem_shape.end() - 2) = num_i32_rows;
  ...
  SmallVector<int64_t> slice_sizes(src_shape.begin(), src_shape.end());
  slice_sizes.back() = lane;
  ...
  for (int i = 0; i < num_i32_rows; ++i) {
    ...
    SmallVector<int64_t> target_vector_shape(i32_view_shape.begin(),
                                             i32_view_shape.end());
